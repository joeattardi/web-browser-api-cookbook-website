...
<div class="p-4">
  <Breadcrumbs>
    ...
    <Breadcrumb href="/examples/{data.chapter.metadata.slug}"
      >{data.chapter.metadata.title}</Breadcrumb
    >
    <Breadcrumb active>{data.metadata.title}</Breadcrumb>
  </Breadcrumbs>

  <small class="text-uppercase"
    >Recipe {data.chapter.metadata.chapter}.{data.metadata.order}</small
  >
